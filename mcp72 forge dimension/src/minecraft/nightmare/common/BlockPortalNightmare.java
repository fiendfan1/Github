...
public class BlockPortalNightmare extends BlockPortalBase implements ISpecialTeleportation{
	...
	// for your portal block.

	public BlockPortalNightmare(int i) {
		super(i, 0/*ModLoader.addOverride("/terrain.png", "/Blocks/ForbiddenPortal.png")*/, Material.portal);
		// Get a unique sprite index for the portal texture so that it doesn't
		// override another
		// texture.
	...
	{
		return new WorldProviderNightmare();
	}

	public Teleporter getTeleporter() 
	{
		return new NightmareTeleporter();
	}
	...
	public List canTeleportFromDimension() 
	{
		...
		arraylist.add(Integer.valueOf(0));// player can teleport from overworld
											// to this dimension
		arraylist.add(Integer.valueOf(-1));// player can teleport from Nether to
											// this dimension
		return arraylist;
	}

	/**
	 * Is this block (a) opaque and ( a full 1m cube? This determines whether or
	...
	public boolean tryToCreatePortal(World par1World, int par2, int par3, int par4) 
	{
		...
		if (par1World.getBlockId(par2 - 1, par3, par4) == mod_Nightmare.nightmareGrass.blockID
				|| par1World.getBlockId(par2 + 1, par3, par4) == mod_Nightmare.nightmareGrass.blockID) {
			i = 1;
		}

		if (par1World.getBlockId(par2, par3, par4 - 1) == mod_Nightmare.nightmareGrass.blockID
				|| par1World.getBlockId(par2, par3, par4 + 1) == mod_Nightmare.nightmareGrass.blockID) {
			j = 1;
		}
		...
		for (int k = -1; k <= 2; k++) {
			for (int i1 = -1; i1 <= 3; i1++) {
				...
				if (flag) {
					if (k1 != mod_Nightmare.nightmareGrass.blockID) {
						return false;
					}
		...
		for (int l = 0; l < 2; l++) {
			for (int j1 = 0; j1 < 3; j1++) {
				par1World.setBlockWithNotify(par2 + i * l, par3 + j1, par4 + j
						* l, mod_Nightmare.nightmarePortal.blockID);
			}
		}
	...
	public void onNeighborBlockChange(World par1World, int par2, int par3, int par4, int par5) {
		...
		if (par1World.getBlockId(par2, k - 1, par4) != mod_Nightmare.nightmareGrass.blockID) {
		...
		if (l != 3
				|| par1World.getBlockId(par2, k + l, par4) != mod_Nightmare.nightmareGrass.blockID) {
			par1World.setBlockWithNotify(par2, par3, par4, 0);
			return;
		}
		...
		if ((par1World.getBlockId(par2 + i, par3, par4 + j) != mod_Nightmare.nightmareGrass.blockID || par1World
				.getBlockId(par2 - i, par3, par4 - j) != blockID)
				&& (par1World.getBlockId(par2 - i, par3, par4 - j) != mod_Nightmare.nightmareGrass.blockID || par1World
						.getBlockId(par2 + i, par3, par4 + j) != blockID)) {
			par1World.setBlockWithNotify(par2, par3, par4, 0);
			return;
		} else {
	...
	public String getEnteringMessage() {
		...
		return "Entering The Forbidden Forest";

	}

	public String getLeavingMessage() {

		return "Exiting The Forbidden Forest";
	}
