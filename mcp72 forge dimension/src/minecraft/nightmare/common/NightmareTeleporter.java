...
public class NightmareTeleporter extends Teleporter {
	// The Teleporter class handles finding existing portals and generating new
	// ones. It pinpoints
	// the location to which the player should be teleported when traveling
	// across dimensions.

	private Random random;

	public NightmareTeleporter() {
		random = new Random();
	}
	...
	public boolean placeInExistingPortal(World world, Entity entity) {
		...
		for (int j1 = l - c; j1 <= l + c; j1++) {
			...
			for (int j2 = i1 - c; j2 <= i1 + c; j2++) {
				...
				for (int k2 = 128 - 1; k2 >= 0; k2--) {
					if (world.getBlockId(j1, k2, j2) != mod_Nightmare.nightmarePortal.blockID) {
						continue;
					}
					for (; world.getBlockId(j1, k2 - 1, j2) == mod_Nightmare.nightmarePortal.blockID; k2--) {
					}
		...
		if (d >= 0.0D) {
			...
			if (world.getBlockId(k1 - 1, l1, i2) == mod_Nightmare.nightmarePortal.blockID) {
				d2 -= 0.5D;
			}
			if (world.getBlockId(k1 + 1, l1, i2) == mod_Nightmare.nightmarePortal.blockID) {
				d2 += 0.5D;
			}
			if (world.getBlockId(k1, l1, i2 - 1) == mod_Nightmare.nightmarePortal.blockID) {
				d6 -= 0.5D;
			}
			if (world.getBlockId(k1, l1, i2 + 1) == mod_Nightmare.nightmarePortal.blockID) {
				d6 += 0.5D;
	...
	public boolean createPortal(World world, Entity entity) {
						...
						for (int j9 = 0; j9 < 3; j9++) {
							for (int k10 = 0; k10 < 4; k10++) {
								for (int l11 = -1; l11 < 4; l11++) {
									...
									if (l11 < 0
											&& !world.getBlockMaterial(j12,
													l12, j13).isSolid()
											|| l11 >= 0
											&& !world.isAirBlock(j12, l12, j13)) {
										break label0;
									}
								}
							}
						...
						label1: for (int l5 = l1; l5 < l1 + 2; l5++) {
							...
							for (int k9 = 0; k9 < 4; k9++) {
								for (int l10 = -1; l10 < 4; l10++) {
									...
									int i13 = k3 + (k9 - 1) * j8;
									if (l10 < 0
											&& !world.getBlockMaterial(i12,
													k12, i13).isSolid()
											|| l10 >= 0
											&& !world.isAirBlock(i12, k12, i13)) {
										break label1;
									}
								}
							}
				...
				for (int i6 = 1; i6 < 3; i6++) {
					for (int j7 = -1; j7 < 3; j7++) {
						...
						world.setBlockWithNotify(
								...
								l9,
								i11,
								flag ? mod_Nightmare.nightmareGrass.blockID
										: 0);
					}
				}
			}
		...
		for (int j5 = 0; j5 < 4; j5++) {
			...
			// You can add your own portal's generation here (and delete all the
			// code up to
			// "world.editingBlocks = false"). This particular portal is like a
			// Nether portal, but with
			// sandstone instead of obsidian, and obviously the Taiga portal
			// block instead of the Nether
			// one.
			for (int j6 = 0; j6 < 4; j6++) {
				for (int k7 = -1; k7 < 4; k7++) {
					...
					world.setBlockWithNotify(
							...
							i10,
							j11,
							flag1 ? mod_Nightmare.nightmareGrass.blockID
									: mod_Nightmare.nightmarePortal.blockID);
				}
			}
			...
			for (int k6 = 0; k6 < 4; k6++) {
				for (int l7 = -1; l7 < 4; l7++) {
					...
					int j10 = i3 + l7;
					int k11 = l3 + (k6 - 1) * j4;
					world.notifyBlocksOfNeighborChange(i9, j10, k11,
							world.getBlockId(i9, j10, k11));
				}
			}
		}
