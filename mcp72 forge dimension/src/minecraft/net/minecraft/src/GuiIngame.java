...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float par1, boolean par2, int par3, int par4)
    {
		...
		{
			for(int overlays = 0; overlays < DimensionAPI.overlayList.size(); overlays++)
			{
				((IRenderOverlay)DimensionAPI.overlayList.get(overlays)).renderGameOverlay(mc, var6, var7);
			}
		}
		/**END EDIT**/
    ...
    {
		/**DIMENSION API EDIT**/
		int texture = Block.portal.blockIndexInTexture;
		String textureOverride = "/terrain.png";
		if(DimensionAPI.isInPortal(mc.thePlayer, Block.portal.blockID))
		{
			DimensionAPI.currentPortal = null;
		}
		if(DimensionAPI.currentPortal != null)
		{
			texture = DimensionAPI.currentPortal.getOverlayTexture();
			textureOverride = DimensionAPI.currentPortal.getOverlayTextureOverride();
		}
		/**END EDIT**/
        ...
        /**DIMENSION API EDIT**/
        GL11.glBindTexture(GL11.GL_TEXTURE_2D, this.mc.renderEngine.getTexture(textureOverride));
        float var4 = (float)(texture % 16) / 16.0F;
        float var5 = (float)(texture / 16) / 16.0F;
        float var6 = (float)(texture % 16 + 1) / 16.0F;
        float var7 = (float)(texture / 16 + 1) / 16.0F;
		/**END EDIT**/
