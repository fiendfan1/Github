...
public class RenderGlobalCustom extends RenderGlobal
{
	...
	public void renderSky(float par1)
    {
		...
		if(!(worldObj.provider instanceof WorldProviderBase))
		{
			super.renderSky(par1);
			return;
		}
		WorldProviderBase worldProvider = (WorldProviderBase)worldObj.provider;
        ...
        if (!worldProvider.isSurfaceWorld()/*func_48217_e()*/)
        {
            return;
        }

        GL11.glDisable(GL11.GL_TEXTURE_2D);
        Vec3/*D*/ vec3d = worldProvider.getSkyColor(worldObj, mc.renderViewEntity, par1);
    ...
    {
		World worldObj = mc.theWorld;
		if(!(worldObj.provider instanceof WorldProviderBase) || (((WorldProviderBase)worldObj.provider).renderClouds()))
		{
